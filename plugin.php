...
//            add_action( 'woocommerce_after_checkout_billing_form', array( $this, 'woocommerce_after_checkout_billing_form' ));
        ...
        /**
         *  Check order limit per user per months
         */
        function woocommerce_after_checkout_billing_form(){
            ?>
            <script type="text/javascript">
                jQuery(document).ready(function () {
                    //jQuery('#place_order').attr('disabled', true);
                    jQuery('body').on('change',"#billing_first_name",function(){

                        if(jQuery(this).val() == '') return;
                        var data = {
                            'action'    : 'check_order_limit',
                            'username'  : jQuery(this).val()
                        };
                        jQuery.post("<?php echo admin_url( 'admin-ajax.php' );?>", data,function(response){
                            if(response.limit){
                                jQuery('#place_order').attr('disabled', true);
                                alert('Pemmission denied. Please remove item '+response.item+' from cart and try again!');
                            }
                        },'JSON');
                    });
                });
        ...
        function my_login_redirect( $redirect_to, $request, $user ) {
            ...
            if ($_COOKIE['custom_redirect']) {
                $redirect_to =  urldecode($_COOKIE['custom_redirect']);
